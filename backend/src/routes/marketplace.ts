...
router.get('/products/:id',
  [param('id').isInt()], getProductsId
);

router.put('/products/:id',
  authenticateToken,
  requireRole(['seller', 'admin']),
  [param('id').isInt()], putProductsId
);

router.delete('/products/:id',
  authenticateToken,
  requireRole(['seller', 'admin']),
  [param('id').isInt()], deleteProductsId
);

router.get('/cart',
  authenticateToken,
  requireRole(['student', 'teacher', 'parent', 'scholar']), getCart
);
...
router.delete('/cart/remove/:productId',
  authenticateToken,
  requireRole(['student', 'teacher', 'parent', 'scholar']),
  [param('productId').isInt()], deleteCartRemoveProductId
);

router.post('/orders/create',
  authenticateToken,
  requireRole(['student', 'teacher', 'parent', 'scholar']),
  checkoutLimiter, postOrdersCreate
);

router.get('/orders',
  authenticateToken, getOrders
);

router.get('/orders/:id',
  authenticateToken,
  [param('id').isInt()], getOrdersId
);
