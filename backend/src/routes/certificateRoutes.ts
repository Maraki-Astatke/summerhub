...
import { generateCertificate, getAllCredentials, getCredentialById } from '../services/certifierService.js';
import prisma from '../lib/prisma.js';

const router = Router();

// ============================================
// DEBUG ENDPOINT - Check student certificates
// ============================================
router.get('/debug/student-certificates', authenticateToken, async (req: any, res: any) => {
    try {
...
// ============================================
// GET ALL STUDENTS (for admin certificate form)
// ============================================
router.get('/admin/students', authenticateToken, requireRole(['admin']), async (req: any, res: any) => {
    try {
...
// ============================================
// ISSUE AI CERTIFICATE TO STUDENT (via Certifier)
// ============================================
router.post('/certificates/issue', authenticateToken, requireRole(['teacher', 'admin']), async (req: any, res: any) => {
    try {
        ...
        if (!student) {
            // ✅ FIXED: Create student with a hashed placeholder password
        ...
        } catch (certifierError: any) {
            // ✅ Fallback: generate a local credential ID so the cert is still saved
...
// ============================================
router.get('/certificates/all', authenticateToken, requireRole(['admin', 'teacher']), async (req: any, res: any) => {
    try {
        ...
        const formattedManual = manualCertificates.map((cert: any) => ({
            id: `manual-${cert.id}`,
            credentialId: null,
            recipient: {
        ...
        const formattedAI = aiCertificates.map((cert: any) => ({
            id: `ai-${cert.id}`,
...
// ============================================
// GET STUDENT'S CERTIFICATES - BOTH MANUAL AND AI WITH AUTO-FIX
// ============================================
router.get('/dashboard/certificates', authenticateToken, requireRole(['student']), async (req: any, res: any) => {
    try {
        ...
        const [manualCerts, aiCerts] = await Promise.all([
            // 1. Manual certificates from Certificate table
            prisma.certificate.findMany({
            ...
            // 2. ✅ AI certificates from CertifierCertificate table
            prisma.certifierCertificate.findMany({
                where: {
                    OR: [
                        { studentId: studentId },
...
// ============================================
// GET STUDENT'S CERTIFICATES (alternative endpoint)
// ============================================
router.get('/certificates/my-certificates', authenticateToken, requireRole(['student']), async (req: any, res: any) => {
    try {
        const studentId = Number(req.user.userId);
...
// ============================================
router.get('/certificates/:id', authenticateToken, async (req: any, res: any) => {
    try {
        const { id } = req.params;
...
// ============================================
router.get('/certificates/:id/download', authenticateToken, async (req: any, res: any) => {
    try {
        const { id } = req.params;
        const studentId = Number(req.user.userId);
        ...
        if (isAI) {
            // Check if it's an AI certificate
            const aiCert = await prisma.certifierCertificate.findUnique({
                where: { id: actualId }
            });
