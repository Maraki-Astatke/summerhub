...
export const postPaymentInitiateOrderId = async (req, res) => {
    try {
        ...
        const payment = await initializePayment(order.user.email, order.totalAmount, order.id, order.user.profile?.firstName || 'Customer', order.user.profile?.lastName || '');
...
export const getPaymentVerifyTxref = async (req, res) => {
    try {
