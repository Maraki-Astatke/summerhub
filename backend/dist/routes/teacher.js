...
router.get('/teacher/stats', authenticateToken, requireRole(['teacher', 'admin']), async (req, res) => {
...
router.get('/teacher/students', authenticateToken, requireRole(['teacher', 'admin']), async (req, res) => {
    const teacherId = req.user.userId;
...
router.get('/teacher/students/:studentId', authenticateToken, requireRole(['teacher', 'admin']), [param('studentId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.get('/teacher/lessons/upcoming', authenticateToken, requireRole(['teacher', 'admin']), async (req, res) => {
    const teacherId = req.user.userId;
    const lessons = await prisma.lesson.findMany({
        where: {
            teacherId,
...
router.post('/teacher/lessons/:id/cancel', authenticateToken, requireRole(['teacher', 'admin']), [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.get('/teacher/revenue', authenticateToken, requireRole(['teacher', 'admin']), async (req, res) => {
    const teacherId = req.user.userId;
...
router.post('/teacher/certificates/upload', authenticateToken, requireRole(['teacher']), upload.single('certificate'), async (req, res) => {
    try {
        if (!req.file) {
...
router.get('/teacher/certificates/issued', authenticateToken, requireRole(['teacher']), async (req, res) => {
    try {
        const teacherId = req.user.userId;
...
router.get('/students/certificates', authenticateToken, requireRole(['student']), async (req, res) => {
    try {
