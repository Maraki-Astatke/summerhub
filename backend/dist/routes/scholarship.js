...
router.get('/scholarships/:id', [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.put('/scholarships/:id', authenticateToken, requireRole(['scholarship_giver', 'admin']), [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.delete('/scholarships/:id', authenticateToken, requireRole(['scholarship_giver', 'admin']), [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.get('/scholarships/applications', authenticateToken, requireRole(['scholarship_giver', 'admin']), async (req, res) => {
