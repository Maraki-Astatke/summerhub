...
router.put('/products/:id', authenticateToken, requireRole(['seller', 'admin']), [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.delete('/products/:id', authenticateToken, requireRole(['seller', 'admin']), [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
// ============================================
// GET cart
router.get('/cart', authenticateToken, requireRole(['student', 'teacher', 'parent', 'scholar']), async (req, res) => {
    const userId = req.user.userId;
...
router.delete('/cart/remove/:productId', authenticateToken, requireRole(['student', 'teacher', 'parent', 'scholar']), [param('productId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.post('/orders/create', authenticateToken, requireRole(['student', 'teacher', 'parent', 'scholar']), checkoutLimiter, async (req, res) => {
    const userId = req.user.userId;
    const cartItems = await prisma.cartItem.findMany({
        where: { userId },
        include: { product: true }
...
router.get('/orders/:id', authenticateToken, [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
