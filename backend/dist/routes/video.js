...
router.get('/video/sessions/:lessonId/join', authenticateToken, [param('lessonId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.post('/video/sessions/:lessonId/end', authenticateToken, requireRole(['teacher', 'admin']), [param('lessonId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.get('/video/recordings/:lessonId', authenticateToken, [param('lessonId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
