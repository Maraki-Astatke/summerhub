...
const router = Router();
router.post('/notifications/send/lesson-reminder', authenticateToken, requireRole(['teacher', 'admin']), [param('lessonId').isInt()], async (req, res) => {
...
router.post('/notifications/send/order-confirmation', authenticateToken, [param('orderId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.post('/notifications/send/welcome', authenticateToken, requireRole(['admin']), [param('userId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.post('/notifications/send/scholarship-approved', authenticateToken, requireRole(['scholarship_giver', 'admin']), [param('applicationId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.post('/notifications/send/certificate-earned', authenticateToken, [param('registrationId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.post('/notifications/send/event-reminder', authenticateToken, [param('eventId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.get('/notifications/history/:userId', authenticateToken, [param('userId').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.put('/notifications/:id/read', authenticateToken, [param('id').isInt()], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
